import os
from hashlib import sha512
from binascii import hexlify

'''
this is only the final run of the solver. I did it somewhat iteratively by hand to identify 'correct' branches where multiple possibilities existed. And yes, it's a ripped off version of the original code because I'm lazy.

-psifertex

'''

#generates s box and sinverse box, called f and g respectively, using 
#sha 512 as a deterministic random number generator
def genTables(seed="Well one day i'll be a big boy just like manhell"):
    fSub={}
    gSub={}
    i=0
    prng=sha512()
    prng.update(seed)
    seed=prng.digest()
    for el in xrange(256):
        cSeed=""
        for x in xrange(4):
            cSeed+=prng.digest()
            prng.update(str(x))
        prng.update(cSeed)
        fCharSub=[0]*256
        gCharSub=[0]*256
        unused=range(256)
        for toUpdate in xrange(256):
            i+=1
            curInd=ord(cSeed[toUpdate])%len(unused)
            toDo=unused[curInd]
            del unused[curInd]
            fSub[(el,toUpdate)]=toDo
            gSub[(el,toDo )]=toUpdate
    return fSub,gSub
f,g=genTables()

def encrypt(pad, ptext):
    assert(len(ptext)<=len(pad))#if pad < plaintext bail
    ctext = []
    if type(ptext)==type(""):
        ptext=map(ord,ptext)
    if type(pad)==type(""):
        pad=map(ord,pad)
    for padByte,ptextByte in zip(pad,ptext):
        ctext.append(f[padByte,ptextByte])
    return  "".join(map(chr,ctext))

def decrypt(pad, ciphertext):
    assert(len(ciphertext)<=len(pad))#if pad < ciphertext bail
    ptext = []
    if type(ciphertext)==type(""):
        ciphertext=map(ord,ciphertext)
    if type(pad)==type(""):
        pad=map(ord,pad)
    for padByte,ctextByte in zip(pad,ciphertext):
        ptext.append(g[padByte,ctextByte])

    return "".join(map(chr,ptext))

def sanity():
    for x in xrange(256):
        for y in xrange(256):
            enc=f[(x,y)]
            dec=g[(x,enc)]
            assert(y==dec)
    for x in xrange(1000):
        toTest=os.urandom(1000)
        pad=os.urandom(1000)
        enc=encrypt(pad,toTest)
        dec=decrypt(pad,enc)
        assert(dec==toTest)
#sanity()
'''
Recovered texts, hex encoded
'''
c=['794d630169441dbdb788337d40fe245daa63c30e6c80151d4b055c18499a8ac3e5f3b3a8752e95cb36a90f477eb8d7aa7809427dde0f00dc11ab1f78cdf64da55cb75924a2b837d7a239639d89fe2b7bc1415f3542dba748dd40', '14a60bb3afbca7da0e8e337de5a3a47ae763a20e8e18695f39450353a2c6a26a6d8635694cbdc34b7d1a543af546b94b6671e67d0c5a8b64db12fe32e275', '250d83a7ed103faaca9d786f23a82e8e4473a5938eabd9bd03c3393b812643ea5df835b14c8e5a4b36cdcfd210a82e2c3c71d27d3c47091bdb391f2952b261fde94a4b23238137a4897d1631b4e18d63', '', '0fc304048469137d0e2f3a71885a5a78e749145510cf2d56157939548bfd5dd7e59dcebc75b678cfeac4cf408fce5dda32c9bfcbfd578bdcb801df32ebf64da365df4b285d5068975137990134bd69991695989b322b0849', '', '', '68c50bd5197bfdbdfa887883783d2455a673a685436915bd72d1af74dffdd2b89df335daee93c36d5f57e147e9a35913d3b3bf33']

def is_ascii(s):
	return all(ord(c) < 127 and ord(c) >= 32 and (not (ord(c)>=35 and ord(c)<44)) for c in s)

def is_ascii(s):
	return all(ord(c) in range(32,127) for c in s)

def is_ascii(s):
	return all(ord(c) in range(32,35)+[39]+range(44,47)+range(48,59)+[63]+range(65,91)+range(97,126) for c in s)

def validbrute(c,pad,printdecrypt=False):
	for entry in c:
		entry=entry.decode('hex')
		padpad=pad.ljust(len(entry))
		decrypted=decrypt(padpad,entry)[:len(pad)]
		if is_ascii(decrypted):
			if printdecrypt:
				print(decrypted)
		else:
			return False
	if printdecrypt:
		print('')
	return True

pad= [223, 71, 40, 139, 212, 234, 111, 104, 57, 164, 228, 134, 113, 43, 190, 155, 143, 97, 127, 64, 231, 114, 248, 102, 115, 59, 15, 116, 229, 175, 124, 86, 169, 69, 119, 30, 191, 235, 195]
while len(pad) < 104:
	possible=[]
	for byte in range(256):
		if validbrute(c,''.join(map(chr,pad+[byte]))):
			print(byte)
			possible.append(byte)
	if len(possible)==1:
		pad+=possible
		continue
	if len(possible)>1:
		print('Warning, multiple possible found: ')
	if len(possible)==0:
		print('Failed to find valid ascii. Correct so far: ')
	for byte in possible:
		print('pad= %s' % repr(pad+[byte]))
		validbrute(c,''.join(map(chr,pad+[byte])),True)
	break
